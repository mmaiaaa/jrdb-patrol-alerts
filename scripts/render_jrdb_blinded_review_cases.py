...
def cached_equal(expected, actual):
    for name in ("surface_candidate_status", "surface_candidate_ego_xy_m",
                 "nearest_supported_surface_m", "inner_roi_projected_points"):
        left, right = expected[name], actual[name]
        if str(left if left is not None else "") != str(right if right is not None else ""):
            raise ValueError(f"Sensor-only preview replay mismatch in {name}: {left!r} vs {right!r}")
...
def render(review, sensor, archive, root, calibration):
    ...
    if int(inside.sum()) != int(sensor["inner_roi_projected_points"]):
        raise ValueError(f"ROI point count changed for case {review['case_id']}")
    selected = np.zeros(len(radii), dtype=bool)
    if sensor["surface_candidate_ego_xy_m"] not in (None, ""):
        value = float(sensor["surface_candidate_ego_xy_m"])
        bands = [band for band in surface_bands(radii[inside])
                 if band[2] >= MIN_BAND_POINTS and round(band[3], 3) == value]
        if len(bands) != 1:
            raise ValueError(f"Cannot recover sensor-selected band for {review['case_id']}")
        lo, hi, _, _ = bands[0]
        selected = inside & (radii >= lo) & (radii < hi)
