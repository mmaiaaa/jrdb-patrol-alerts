#!/usr/bin/env python3
"""Audit a four-frame JRDB development-only surface/annotation comparison.
...
def main():
    ...
    for ref in references:
        ...
        if key in (row["key"] for row in linked):
    ...
    linked_keys = {row["key"] for row in linked}
    ...
    result = {"status": "development_paired_center_discrepancy_triage_only",
              ...
              "input_frame_ids": sorted(per_frame),
              ...
              "near_pairs_by_frame": dict(sorted(Counter(r["key"][0] for r in near_rows).items())),
              "per_frame": {frame: compare(rows) for frame, rows in sorted(per_frame.items())},
              ...
              "interpretation": "Paired discrepancies use the SAME matched frame-camera-box "
                  ...
                  "four-frame dev sample as an event, held-out, or system-level result."}
