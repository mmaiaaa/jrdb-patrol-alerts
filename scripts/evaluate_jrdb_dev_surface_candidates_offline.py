...
def main():
    ...
    parser.add_argument("--data-root", type=Path,
                        default=Path("data/jrdb2022/development"))
    parser.add_argument("--labels-zip", type=Path,
                        default=Path.home() / "Downloads/labels.zip")
    parser.add_argument("--candidates", type=Path,
                        default=Path("outputs/local/cubberly_sensor_surface_candidates.csv"))
    parser.add_argument("--candidate-summary", type=Path,
                        default=Path("outputs/local/cubberly_sensor_surface_summary.json"))
    parser.add_argument("--csv", type=Path,
                        default=Path("outputs/local/cubberly_surface_reference_diagnostic.csv"))
    args = parser.parse_args()
    manifest = json.loads(args.manifest.read_text())
    if manifest.get("role") != "development_only" or manifest.get("sequence") != SEQUENCE:
        parser.error("Only Cubberly development references are accepted")
    run = json.loads(args.candidate_summary.read_text())
    if (run.get("status") != "unvalidated_measured_surface_candidates" or
            run.get("sequence") != SEQUENCE or run.get("role") != "development_only"):
        parser.error("Candidate summary differs from expected development pilot")
    frames = run["frames"]
    ...
    print(json.dumps({"status": "development_annotation_center_discrepancy_only",
