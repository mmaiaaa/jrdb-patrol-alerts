#!/usr/bin/env python3
"""Development-only, paired label-assisted review of exploratory 3 m crossings.
...
def paired_concordance(pairs):
    """Compare both sensor radii on the exact same linked camera-box views.

    Concordance is with an annotation *center* threshold, not true surface or
    event status. Camera views and frames are correlated, so no CI is implied.
    """
    outcomes = Counter()
    for frame, label_id, center, candidate, nearest in pairs:
        ref_inside = center <= RADIUS_M
        cand_inside = candidate <= RADIUS_M
        nearest_inside = nearest <= RADIUS_M
        candidate_concordant = cand_inside == ref_inside
        nearest_concordant = nearest_inside == ref_inside
        outcomes["candidate_inside_3m_views"] += cand_inside
        outcomes["nearest_inside_3m_views"] += nearest_inside
        outcomes["candidate_center_threshold_concordant_views"] += candidate_concordant
        outcomes["nearest_center_threshold_concordant_views"] += nearest_concordant
        if candidate_concordant and nearest_concordant:
            outcomes["both_concordant_views"] += 1
        elif candidate_concordant:
            outcomes["candidate_only_concordant_views"] += 1
        elif nearest_concordant:
            outcomes["nearest_only_concordant_views"] += 1
        else:
            outcomes["neither_concordant_views"] += 1
    return {"same_view_pairs": len(pairs),
            "distinct_linked_frame_person_ids": len({(frame, label_id) for frame,
                                                       label_id, *_ in pairs}),
            **{name: outcomes[name] for name in (
                "candidate_inside_3m_views", "nearest_inside_3m_views",
                "candidate_center_threshold_concordant_views",
                "nearest_center_threshold_concordant_views",
                "both_concordant_views", "candidate_only_concordant_views",
                "nearest_only_concordant_views", "neither_concordant_views")}}
...
def main():
    ...
    for row in refs:
        ...
        nearest = finite_optional(row["nearest_supported_surface_m"], "nearest")
        ...
        if candidate is not None and nearest is not None:
            paired.append((k[0], g[1], center, candidate, nearest))
    ...
    print(json.dumps({
        ...
        "paired_center_threshold_concordance": {
            "all": paired_concordance(paired),
            "center_within_3m": paired_concordance([
                pair for pair in paired if pair[2] <= RADIUS_M]),
            "center_beyond_3m": paired_concordance([
                pair for pair in paired if pair[2] > RADIUS_M]),
            "center_within_0_25m_of_threshold": paired_concordance([
                pair for pair in paired if abs(pair[2] - RADIUS_M) <= MARGIN_M]),
            "center_farther_than_0_25m_from_threshold": paired_concordance([
                pair for pair in paired if abs(pair[2] - RADIUS_M) > MARGIN_M]),
        },
        ...
        "interpretation": "Exploratory 3m threshold discordance conditional on IoU-matched "
            ...
            "centers differ physically; paired threshold concordance and crossing "
            "counts are not person-range accuracy or validated alert errors. "
