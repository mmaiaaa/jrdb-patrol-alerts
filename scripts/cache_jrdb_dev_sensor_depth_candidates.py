...
def main():
    ...
    if manifest.get("sequence") != SEQUENCE or manifest.get("role") != "development_only":
        parser.error("Only the Cubberly development manifest is accepted")
    if (not args.frames or len(args.frames) != len(set(args.frames)) or any(
            not s.isdecimal() or len(s) != 6 or not 0 <= int(s) <= 1295 for s in args.frames)):
        parser.error("--frames requires distinct Cubberly six-digit IDs from 000000 to 001295")
    if args.csv.exists():
