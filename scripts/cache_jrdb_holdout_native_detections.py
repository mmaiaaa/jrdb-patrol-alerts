#!/usr/bin/env python3
"""Cache holdout-only, five-camera person detections from raw JRDB image ZIP.
...
def main():
    ...
    parser.add_argument("--max-frames", type=int, default=0, help="0 = all remaining holdout frames")
    ...
    if (
        manifest.get("role") != "holdout_sensor_only_unscored"
        or manifest.get("sequence") != "memorial-court-2019-03-16_0"
    ):
        parser.error(
    ...
    with zipfile.ZipFile(args.images_zip) as archive:
        ...
        summary = {
            "sequence": sequence, "role": "holdout_sensor_only_unscored", "source": "train_images.zip",
